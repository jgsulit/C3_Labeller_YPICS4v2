<?php
	...
	function save_new_c3_label_format30(){
		require_once('../class/c3.php');
		$return 		= $_POST; 
		$print_qty      = $return['txt_print_qty'];
		$return['query'] = '';
		...
		for($x=0;$x<$print_qty;$x++){
			if($x==0){
				...
				$lotno_extension = generate_new_lot_no_extension_format30($lotno_extension);
			}
			$result 		= "";
			...
			$result 		= C3::getInstance()->insert_query($table,$array_fields,$array_values);
			$return["result"] = json_encode($result); //return the the serialize values
			/* generate zebra code format 30 */
			$return['label'][] = generate_zebra_code_format30($return['txt_pn'],$return['txt_mpn'],$return['txt_qty'],$return['txt_date'],$return['txt_lot_no'],$lotno_extension);
		}
		$return["msg"]  = 1;
		echo json_encode($return); //json encode since I used a dataType json before passing the serialized values here
	}
	...
	function reprint_label_format30(){
		...
		$return['label'] = '';
		while($row = mysqli_fetch_array($result)){
			$label = generate_zebra_code_format30($row['pn'],$row['mpn'],$row['qty'],$row['date'],$row['lotno'],$row['lotno_extension']);
			$return['label'][] = $label;
		}
		// $return['msg'] = $result;
		$return['result'] = json_encode($result);
