<?php
	if(is_ajax()) {
		if(isset($_POST["action"]) && !empty($_POST["action"])) {
			...
			switch($action) {
				...
				/* format 29 */
				case "check_existing_item_name_and_lotno_format29"	: check_existing_item_name_and_lotno_format29(); break; 
				case "save_new_c3_label_format29"					: save_new_c3_label_format29(); break;
				case "load_selected_label_info_format29"				: load_selected_label_info_format29(); break;
				case "delete_label_format29"							: delete_label_format29(); break;
				case "reprint_label_format29"						: reprint_label_format29(); break;
	...
	/***************************************************************/
		...
		// function save_new_c3_label_format29(){
	// 	require_once('../class/c3.php');
	// 	$return 		= $_POST; 
	// 	$print_qty      = $return['txt_print_qty'];
	// 	$return['query'] = '';
	...
	// 		$return["result"] = json_encode($result); //return the the serialize values
	// 		/* generate zebra code format 6 */
	// 		$return['label'][] = generate_zebra_code_format29($return['txt_pn'],$return['txt_mpn'],$return['txt_qty'],$return['txt_date'],$return['txt_lot_no'],$lotno_extension);
	// 	}
	// 	$return["msg"]  = 1;
	// 	echo json_encode($return); //json encode since I used a dataType json before passing the serialized values here
	// }
	
	function check_existing_item_name_and_lotno_format29(){
		require_once('../class/c3.php'); 
		$return = $_POST;
		$pn = $return['pn'];
		$lotno = explode("-",$return['lotno']);
		...
		if($return['lotno_extension'] == ''){
			...
			$return['lotno_extension'] = generate_new_lot_no_extension_format29($return['lotno_extension']);
		}
		$return['result'] = json_encode($result);
		echo json_encode($return);
	}
	
	function generate_new_lot_no_extension_format29($lot_extension){
		$new_lot_no_extension = $lot_extension[0].$lot_extension[1];
		$letter_end = $lot_extension[2];
		$new_lot_no_extension = str_pad(($new_lot_no_extension + 1), 2, "0", STR_PAD_LEFT); 
		$new_lot_no_extension .= $letter_end;
		...
		for($x=0;$x<$print_qty;$x++){
			...
			$return['checking']['lotno_extension'][] 	= $lotno_extension;
			$result 		= "";
			$table   		= "tbl_format29"; //table name
			$array_fields 	= array('pn','mpn','qty','date','lotno','lotno_extension','date_encoded','lastupdate','username','emp_number','partial','pats','format_no'); //table fields
			$array_values 	= array($return['txt_pn'],$return['txt_mpn'],$return['txt_qty'],$return['txt_date'],$return['txt_lot_no'],$lotno_extension,date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$return['username'],strtoupper($return['txt_employee_number_scanner']),(($return['partial'])=='true'?1:0),(($return['pats'])=='true'?1:0),'29' ); //values to be saved
			$result 		= C3::getInstance()->insert_query($table,$array_fields,$array_values);
			$return["result"] = json_encode($result); //return the the serialize values
			
			/* generate zebra code format 29 */
			$return['label'][] = generate_zebra_code_format29($return['txt_pn'],$return['txt_mpn'],$return['txt_qty'],$return['txt_date'],$return['txt_lot_no'],$lotno_extension);
		}
		$return["msg"]  = 1;
		echo json_encode($return); //json encode since I used a dataType json before passing the serialized values here
	...
	function load_selected_label_info_format29(){
		require_once('../class/c3.php'); 
		$return = $_POST;
		$array_pkid = $_POST['array_pkid'];
		$result = "";
	...
	function delete_label_format29(){
		require_once('../class/c3.php'); 
		$return = $_POST; 
		$array_pkid = $_POST['array_pkid'];
		$return['msg'] = '';
	...
	function reprint_label_format29(){
		require_once('../class/c3.php'); 
		$return = $_POST;
		$array_pkid = $_POST['array_pkid'];
		$result = "";
		...
		$return['label'] = '';
		while($row = mysqli_fetch_array($result)){
			$label = generate_zebra_code_format29($row['pn'],$row['mpn'],$row['qty'],$row['date'],$row['lotno'],$row['lotno_extension']);
			$return['label'][] = $label;
		}
		// $return['msg'] = $result;
		$return['result'] = json_encode($result);
	...
	function generate_zebra_code_format29($pn,$mpn,$qty,$date,$lotno,$lotno_extension){
		
		if ( $mpn == 'FPS009-2903-0-10'){
			$mpn_rev ='FPS009-2903-0(10)';
		}else if ( $mpn == 'CN176S-060-0002-01'){
			$mpn_rev ='CN176S-060-0002(01)';
		}else if ( $mpn == 'CA009-S101-001-08'){
			$mpn_rev ='CA009-S101-001(08)';
		}else{
			$mpn_rev = $mpn;
		}
		...
		$font_style_name 	= $space['font_style_name'];
		$zebra_code = "^XA
						^FO".( 90+ $left_space ).", ".( 40+ $top_space )."
						^CF 0,30
						^FD<P/N> ".$mpn."
						^FS

						^FO".( 90+ $left_space ).", ".( 80+ $top_space )."
						^CF 0,30
						^FD<S/N>  ".$lotno."-".$lotno_extension."
						^FS

						^FO".( 490+ $left_space ).", ".( 80+ $top_space )."
						^CF 0,30
						^FD<QTY>  ".$qty." pcs
						^FS

						^FO".( 90+ $left_space ).", ".( 120+ $top_space )."
						^CF 0,30
						^FD(3N)1 ".$mpn."
						^FS

						^FO".( 90+ $left_space ).", ".( 150+ $top_space )."
						^BY2
						^BCN, 30, N, N, N
						^FD3N1".$mpn."
						^FS

						^FO".( 90+ $left_space ).", ".( 190+ $top_space )."
						^CF 0,30
						^FD(3N)2 ".$qty." ".$lotno."-".$lotno_extension." ".$pn."
						^FS

						^FO".( 90+ $left_space ).", ".( 220+ $top_space )."
						^BY2
						^BCN, 30, N, N, N
						^FD3N2 ".$qty." ".$lotno."-".$lotno_extension." ".$pn."
						^FS

						^FO".( 90+ $left_space ).", ".( 265+ $top_space )."
						^CF 0,30
						^FDYAMAICHI ELECTRONICS Co., Ltd.
						^FS

						^FO".( 90+ $left_space ).", ".( 300+ $top_space )."
						^CF 0,30
						^FD<P/N> ".$mpn_rev."
						^FS

						^FO".( 90+ $left_space ).", ".( 345+ $top_space )."
						^CF 0,30
						^FD[EIAJ C-3]
						^FS
