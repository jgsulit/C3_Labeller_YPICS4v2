<?php
	...
	function generate_zebra_code_format8($pn_code,$qty,$yec_pn,$lotno,$lotno_extension){
		...
		$zebra_code = "^XA
							...
							^FO".( 570+ $left_space ).", ".( 230+ $top_space )."
							^BQN,2, 5
							^FD".add_backslash_reserve_words($yec_pn)." ".$pn_code." ".$lotno."-".$lotno_extension."
							^FS
							...
							^XZ";
							// ^FO".( 570+ $left_space ).", ".( 265+ $top_space )." OLD 3-24-2023 CHAN
		return $zebra_code;
	}
	...
	function generate_zebra_code_format16($item_name,$qty,$lotno,$lotno_extension,$order_no,$remarks){
		...
		// $zebra_code.=	"^FO".( 620+ $left_space ).", ".( 65+ $top_space ); //01-30-23 cbretusto
		// $zebra_code.=	"^FO".( 620+ $left_space ).", ".( 170+ $top_space ); //04-05-2023 cbretusto
		// $zebra_code.=	"^FO".( 620+ $left_space ).", ".( 100+ $top_space ); //04-11-2023 cdcasuyon
		$zebra_code.=	"^FO".( 620+ $left_space ).", ".( 65+ $top_space ); 
		$zebra_code.=	"^BQN,2, 5";
		$zebra_code.=	"^FD\\\\".$lotno."-".$lotno_extension;
		$zebra_code.=	"^FS";
	...
	// 					^FX### LOT NO QR CODE ###
	// 					^FO".( 620+ $left_space ).", ".( 88+ $top_space )."
	// 					^BQN,2, 5
	// 					^FD\\\\".$lotno."-".$lotno_extension."
	// 					^FS
	...
	*/
	...
	function search_item_ts(){
		...
		while($row = mssql_fetch_array($result)){
			...
			$prodname = $row['NAME'];
			if( strpos( $prodname, "Burn-in" ) !== false) {
				$temp = explode('-', $prodname);
				unset($temp[count($temp) - 1]);
				unset($temp[count($temp) - 1]);
				$prodname = implode('-', $temp);
