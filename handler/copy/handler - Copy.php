<?php
	...
	function save_new_c3_label_format21(){
		...
		// for($x=0;$x<$print_qty;$x++){
			...
			$result 		= "";
			$table   		= "tbl_format6"; //table name
			$array_fields 	= array('pn','mpn','qty','date','lotno','date_encoded','lastupdate','username'); //table fields
			$array_values 	= array($return['txt_pn'],$return['txt_mpn'],$return['txt_qty'],$return['txt_date'],$return['txt_lot_no'],date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$return['username']); //values to be saved
			$result 		= C3::getInstance()->insert_query($table,$array_fields,$array_values);
			$return["result"] = json_encode($result); //return the the serialize values
			/* generate zebra code format 6 */
