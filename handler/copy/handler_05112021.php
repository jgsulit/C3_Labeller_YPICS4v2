<?php
	...
	function generate_zebra_code_format18($product_id,$size){
		...
		if($size == 1){
			...
			$zebra_code = "	^XA
							...
							^FO".(  1+ $left_space ).", ".( $top_space-4 )."
							^GB 1540,488,8,B
							^FS
							^CF 0,80

							^FO".(  1+ $left_space ).", ".( $top_space-3 )."
							^GB 1540,209.2,4,B
	...
	function save_new_c3_label_format21(){
		...
		for($x=0;$x<$print_qty;$x++){
			...
			$result 		= "";
			$table   		= "tbl_format6"; //table name
			$array_fields 	= array('pn','mpn','qty','date','lotno','date_encoded','lastupdate','username','emp_number','partial'); //table fields
			$array_values 	= array($return['txt_pn'],$return['txt_mpn'],$return['txt_qty'],$return['txt_date'],$return['txt_lot_no'],date('Y-m-d H:i:s'),date('Y-m-d H:i:s'),$return['username'],strtoupper($return['txt_employee_number_scanner']),(($return['partial'])=='true'?1:0)); //values to be saved
			$result 		= C3::getInstance()->insert_query($table,$array_fields,$array_values);
			$return["result"] = json_encode($result); //return the the serialize values
			/* generate zebra code format 6 */
