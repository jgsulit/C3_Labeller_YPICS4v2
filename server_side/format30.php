<?php
	...
	*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$row[] = '<input type="hidden" id="hd_pkid" value="'.$aRow['pkid'].'">'.$aRow['mpn'];
		$row[] = $aRow['qty'];
		if($aRow['lotno_extension'] <= 9){
			$new_lot_no_extension = str_pad($aRow['lotno_extension'],2,"0",STR_PAD_LEFT);
		}else{
			$new_lot_no_extension = $aRow['lotno_extension'];
		}
		$row[] = $aRow['lotno'].'-'.$new_lot_no_extension.$aRow['series'];
		$row[] = $aRow['date'];
		$row[] = $aRow['emp_number'];
		$row[] = $aRow['partial']?'Yes':'No';
