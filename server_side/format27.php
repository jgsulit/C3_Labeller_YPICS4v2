...
select * from vw_format27 where item_name != 'HF301S-106-1800-4021-VE'
...
vw_format27
	...
	*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		$row[] = '<input type="hidden" id="hd_pkid" value="'.$aRow['pkid'].'">'.$aRow['item_name'];
		$row[] = $aRow['qty'];
		$row[] = $aRow['order_no'];
		$row[] = $aRow['lotno'].'-'.$aRow['lotno_extension'];
		$row[] = $aRow['remarks'];
		$row[] = $aRow['date_encoded'];
		$row[] = $aRow['emp_number'];
