<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 11
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		...
		<div id="container_label">
			<table id="tbl_format11" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_partno" id="txt_partno" tabindex="1" required>
					<label class="control-label">Vendor:</label>
					<input type="text" class="form-control" name="txt_vendor" id="txt_vendor" tabindex="3">
...
var oTable = $('#tbl_format11').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource"		: "../server_side/format11.php"
} );
...
$('#txt_lot_no').change(function(){
	...
	var lotno 		= $(this).val();
	check_existing_item_name_and_lotno(partno,lotno);
});

/* function for checking existing item name and lotno */
function check_existing_item_name_and_lotno(partno,lotno){
	var data = {
		"action" 		: "check_existing_item_name_and_lotno_format11",
		"partno"		: partno,
		"lotno"			: lotno
	}
	data = $.param(data);
...
$('#frm_new_label').submit(function(e){
	e.preventDefault();
	var data = {
		"action"	: "save_new_c3_label_format11",
		"username"	: username
	}
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format11",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format11 tbody').on('click','#chk_select', function(){
