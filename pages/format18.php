<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> UL Label (70mm x 20mm)
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		<div class="btn-container">
			<p class="align-right">
				<select id="txt_select_printer" class="" style="height: 28px;border-radius: 5px;">
					<option value="">-- select printer --</option>
					<option value="Zebra ZT420 (300 dpi)">PPC Kiosk - Zebra ZT420 (300 dpi)</option>
					<option value="ZDesigner ZT420-300dpi ZPL">Charlie - ZDesigner ZT420-300dpi ZPL</option>
				</select>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_product_id" id="txt_product_id" tabindex="1" required>
					<label class="control-label">Label Size:</label><br />
					<input type="radio" name="txt_size" id="txt_size" tabindex="2" value="1" required> 70mm x 20mm
					<input type="radio" name="txt_size" id="txt_size-2" tabindex="3" value="2" required> 130mm x 42mm
				</div>
				<div class="col-sm-6">
					<label class="control-label">Print Qty:</label>
					<div class="row">
						<div class="col-sm-4">
							<input type="number" class="form-control" name="txt_print_qty" id="txt_print_qty" tabindex="4" value="1" required>
						</div>
						<!--<div class="col-sm-5">
...
var oTable = $('#tbl_format1').DataTable( {
	...
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource": "../server_side/format18.php"
} );
...
$('#frm_new_label').submit(function(e){
	...
	$.ajax({
		...
		success	: function(result){
			...
			// $('#frm_new_label input').val('');//comment temporary 1/30/2019
			$('#frm_new_label input[type="number"]').val('');
		}
	});
});
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info18",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
function search_item(search_category,search_pattern){
	$.ajax({
		...
		success	: function(result){
			...
			$('#txt_product_id').val(result['item_name'][0]);
...
function print_labels(array_label){
	if( $('#txt_select_printer').val()=='' ){
		alert('Please select pritner.')
	}
	else{
		findPrinter( $('#txt_select_printer').val() ,array_label);
	}
	// if( $('#modal_new_sticker #txt_size').prop('checked',true)  ){
		// alert('70mm x 20mm');
		// findPrinter('ZT220',array_label);
	// }else{
		// findPrinter('ZT420',array_label);
	// }
}
</script>
