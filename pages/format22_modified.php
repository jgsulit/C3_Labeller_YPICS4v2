...
$_SESSION['pats_txt_employee_number_scanner'] = isset($_GET['en'])?$_GET['en']:'';
?>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_itemname" id="txt_itemname" tabindex="1" required readonly>
			...
			<div id="sticker_preview">
				<p class="align-center"><img src="../img/C3_Label_Format/format22.jpg" class="image"></p>
		...
		<div class="modal-content">
			...
			<div class="modal-body pt-0">
				<div class="text-center text-secondary">
					...
					<?php
						if( $_SERVER['REMOTE_ADDR'] == '192.168.22.14' ){//PPC kiosk, no scanner
							echo '<button type="button" class="btn btn-success btn-sm" id="btn_ppc_save">PPC? Just Press Enter</button>';
						}
					?>
				</div>
...
var oTable = $('#tbl_format1').DataTable( {
	...
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource": "../server_side/format22.php"
} );
...
$('#frm_new_label').submit(function(e){
	...
	//---
	//novs 2020-12-22
	var pats_txt_employee_number_scanner = "<?php echo $_SESSION['pats_txt_employee_number_scanner']; ?>";
	if( $('#btn_ppc_save').length ){
		$('#txt_employee_number_scanner').val( pats_txt_employee_number_scanner );
	}
...
function frm_new_label(){
	var data = {
		"action"	: "save_new_c3_label_format22",
		"username"	: username,
		...
		success : function(data3){
			...
			if($.trim(data3)){
				if(jQuery.inArray(data3[0]['position'], position) !== -1){
					...
					alert_msg = 'Please scan Supervisor, PPC or QC Inspector\'s ID for partial qty.';
	...
	$.ajax({
		...
		success	: function(result){
			...
			if(result['item_name'][0]!='HF301S-106-1800-4021-VE'){
				alert('The PO is not for HF301S-106-1800-4021-VE');
				return false;
			}
