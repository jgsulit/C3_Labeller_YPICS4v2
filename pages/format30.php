<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 30
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		<div class="btn-container">
			<p class="align-right">
				...
				<?php session_start(); ?>
				<?php								
					// if($_SESSION['username'] == 'jojpato' || $_SESSION['username'] == 'jangeco'){
		...
		<div id="container_label">
			<table id="tbl_format30" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_pn" id="txt_pn" tabindex="1"  required readonly>
					
					<label class="control-label">Manufacture P/N:</label>
					<input type="text" class="form-control" name="txt_mpn" id="txt_mpn" tabindex="3" readonly>
					...
					<div class="row">
						<div class="col-sm-6">
					        <?php								
								...
								// }
							?>    
<!--                            <input type="number" class="form-control" name="txt_qty" id="txt_qty" tabindex="2" value="240" readonly required>-->
						</div>
					</div>					
					...
					<div class="row" id="container_lot_number">
						...
						<div class="col-sm-4"> 
							<input type="text" class="form-control" name="txt_lot_no_count" id="txt_lot_no_count" placeholder="----" required readonly>
						</div>
					</div>
					<label class="control-label">Print Qty:</label>
					<label class="control-label" style="margin-left:30%;">A/B:</label>
					<label class="control-label" style="margin-left:6%;">C/D:</label>
					<div class="row">
						<div class="col-sm-4">
							<input type="number" class="form-control" name="txt_print_qty" id="txt_print_qty" tabindex="6" value="1" min="1" max = "10" required>
						</div>
						<div class="col-sm-2">
						</div>
						<div class="col-sm-2">
							<input type="radio" class="form-control" name="chck_option_series" id="chck_option_a_b" tabindex="6" value="1" disabled required>
						</div>
						<div class="col-sm-2">
							<input type="radio" class="form-control" name="chck_option_series" id="chck_option_c_d" tabindex="6" value="2" disabled required>
						</div>
					</div>
				</div>
			</div>
			...
			<div id="sticker_preview">
				<p class="align-center"><img src="../img/C3_Label_Format/format30.png" class="image"></p>
			</div>
			<br>
			...
			<div class="alert alert-warning" style="padding: 3px 10px; display: none;">
				<label class="control-label">
					<input type="checkbox" class="" name="ckb_pats" id="ckb_pats" tabindex="">
					Print for PATS
				</label>
			</div>
      </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="frm_re_print_label">
			...
			<div id="" style="opacity:0;"> <!-- Note: In case they  request to have modify--->
				<label for="">Print Qty: </label>
				<input class="form-control" type="number" name="int_reprint_qty" value="1" min ="1" max="10" required>
			</div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default fa fa-close" data-dismiss="modal">Close</button>
...
<!--
	Added by -Chris
-->
<div class="modal fade" id="mdl_employee_number_scanner_for_supervisor" data-formid="" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
	<div class="modal-dialog modal-sm modal-dialog-centered" role="document">
		<div class="modal-content">
			<div class="modal-header border-bottom-0 pb-0">
				<!-- <h5 class="modal-title" id="exampleModalLongTitle"></h5> -->
			...
			<div class="modal-body pt-0">
				...
				<input type="text" id="txt_employee_number_scanner_for_supervisor" autocomplete="off" class="hidden_scanner_input" style="position: absolute;opacity:0;" required>
...
var handler = '../handler/handler_F30.php';
...
var oTable = $('#tbl_format30').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource"		: "../server_side/format30.php"
} );
...
$('#btn_create_new_label').click(function(){
	...
	$('#txt_qty').attr('data-qty',0);
	/* display modal */
	$('#modal_new_sticker').modal({backdrop: 'static',
		keyboard: false},'show');
});
/**nmodify */
$('#txt_print_qty').keyup(function (e) { 
	let print_qty = $(this).val();
	if(print_qty > 10){ /* The maximum standard for new printing is 10 only */
		$(this).val(10);
	}
});
$('#txt_date').keyup(function (e) { 
	let date_code = $(this).val();
	// $('#txt_lot_no').val(date_code);

	if( e.keyCode != 13){ /*Note: If the input field is not been enter the value of lot extension is null and unchecked radio button */
		$('[name="txt_lot_no_count"]').val('');
		$('[name="chck_option_series"]').removeAttr('checked',true);
	}
	if(date_code!=""){
		$('[name="chck_option_series"]').prop('disabled',false);
	}else{
		$('[name="chck_option_series"]').prop('disabled',true);
	}
	// $('[name="txt_lot_no_count"]').val('');
});
$('#txt_lot_no').keyup(function (e) { 
	let lot_no = $(this).val();
	$('#txt_date').val(lot_no);
	if( e.keyCode != 13){ /*Note: If the input field is not been enter the value of lot extension is null and unchecked radio button */
		$('[name="txt_lot_no_count"]').val('');
...
$('[name="chck_option_series"]').click(function () { 
	let pn 			= $('#txt_pn').val();
	let lotno 		= $('#txt_lot_no').val();
	let series 		= $(this).val();
	check_existing_item_name_and_lotno(pn,lotno,series);

});
function check_existing_item_name_and_lotno(pn,lotno,series){
	
	var data = {
		"action" 	: "check_existing_item_name_and_lotno_format30",
		"pn"		: pn,
		"lotno"		: lotno,
		"series"	: series
	}
	...
	$.ajax({
		...
		success	: function(result){
			...
			if (result['last_series']!=undefined) {
				$('#txt_lot_no_count').val(result['lotno_extension']+result['last_series']);
			}else{
...
$('#frm_new_label').submit(function(e){ 
	...
	//---
		...
		if( $('#ckb_partial').is(':checked') || $('#ckb_pats').is(':checked') ){
...
function frm_new_label(){ 
	var data = {
		"action"	: "save_new_c3_label_format30",
		"username"	: username,
		...
		success : function(data3){
			...
			if($.trim(data3)){
				if(jQuery.inArray(data3[0]['position'], position) !== -1){
					...
					alert_msg = 'Please scan Supervisor or QC Inspector\'s ID.';// for partial qty or for PATS
...
$(document).on('keyup',function(e){
	if( ($("#mdl_employee_number_scanner").data('bs.modal') || {}).isShown ){
		...
		if( e.keyCode == 13 ){
		  	console.log( $("#txt_employee_number_scanner").val() );
...
$('#btn_reprint_label').click(function(){
	...
	if(array_selected.length == 0){
		...
		return false;
	}else if(array_selected.length>1){
		$('#frm_re_print_label input[name="int_reprint_qty"]').prop('disabled',true);
		// return false;
	}else{
		$('#frm_re_print_label input[name="int_reprint_qty"]').prop('disabled',false);
		// return true;
	}
...
$('#frm_re_print_label').submit(function(e){
	e.preventDefault();
	/* do reprinting here */
	$('#mdl_employee_number_scanner_for_supervisor').modal('show');
...
/* ========= event for scanning of supervisor id for reprint ============ */
$(document).on('keypress', function(e){
	if( ($("#mdl_employee_number_scanner_for_supervisor").data('bs.modal') || {}).isShown ){
		$('#txt_employee_number_scanner_for_supervisor').focus();
		// setTimeout(() => {
		// 	$('#txt_employee_number_scanner_for_supervisor').val('');
		// }, 200);
		

		if(e.keyCode == 13){
			
			// console.log('supervisor Id: ',$('#txt_employee_number_scanner_for_supervisor').val());
			let empId = $('#txt_employee_number_scanner_for_supervisor').val();
			validate_user_for_supervisor(['1','2','7','8'],empId);
			$('#txt_employee_number_scanner_for_supervisor').val('');
		}
	}
	// $('#txt_employee_number_scanner_for_supervisor').val('');
...
function validate_user_for_supervisor(position, empId){
	var data2 = {
		'employee_id' 				: empId,
		'status' 					: 1,
	};
	...
	$.ajax({
		...
		success: function (response) {
			...
			if($.trim(response)){
				...
				alert('User ID not found in PATS.');
			}
		}
	});
	// console.log(data2);
...
function reprint_label(){
	reprint_qty = $('#frm_re_print_label input[name="int_reprint_qty"]').val();
	
	var data = {
		"action"	 	: "reprint_label_format30",
		"array_pkid" 	: array_selected,
		"reprint_qty" 	: reprint_qty,
	}
	data = $.param(data);
	$.ajax({
		type	: "post",
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format30",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format30 tbody').on('click','#chk_select', function(){
...
function search_item(search_category,search_pattern){
	...
	$('#txt_mpn').val('');
	// $('#txt_lot_no').val('');
	// $('#txt_lot_no_count').val('');
	$('#txt_print_qty').val('1');
	$('#ckb_partial').prop('checked',false);
	$('#txt_qty').attr('data-qty',0);
	//reset----
	...
	$.ajax({
		...
		success	: function(result){
			...
			// if(result['item_name'][0]!='CA009-1204-001(H1)'){
				// alert('The PO is not for CA009-1204-001(H1)');
			// 	return false;
			// }
...
function default_values(device_name){
	$.ajax({
		...
		success	: function(result){
			...
			$('#txt_qty').attr('data-qty',result['data']['quantity']);
...
function get_work_week(){
	$.ajax({
		...
		success	: function(result){
			...
			// $('#txt_lot_no').val(result['work_week']);
		}, error : function(){
			alert('error handler');
		}
	});
