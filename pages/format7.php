<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 7
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		...
		<div id="container_label">
			<table id="tbl_format7" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_pn_code" id="txt_pn_code" tabindex="1" readonly required>
					<label class="control-label">Contractor Code:</label>
					<input type="text" class="form-control" name="txt_contractor_code" id="txt_contractor_code" tabindex="3" value="108205" readonly>
					<label class="control-label">Spec:</label>
					<input type="text" class="form-control" name="txt_spec" id="txt_spec" tabindex="6" readonly>
...
var oTable = $('#tbl_format7').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource"		: "../server_side/format7.php"
} );
...
function check_existing_item_name_and_lotno(spec,lotno){
	var data = {
		"action" 		: "check_existing_item_name_and_lotno_format7",
		"spec"			: spec,
		"lotno"			: lotno
	}
	data = $.param(data);
...
$('#frm_new_label').submit(function(e){
	e.preventDefault();
	var data = {
		"action"	: "save_new_c3_label_format7",
		"username"	: username
	}
	...
	$.ajax({
		...
		success	: function(result){
			...
			$('#frm_new_label #txt_contractor_code').val('108205');
		}
	});
});
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format7",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format7 tbody').on('click','#chk_select', function(){
...
function search_item(search_category,search_pattern){
	$.ajax({
		...
		success	: function(result){
			...
			$('#txt_spec').val(result['item_name'][0]);
			if(result['item_name'][0] == "FMS006-2310-0(03)"){
				$('#txt_pn_code').val("QSOCZA372WJQZY");
			}else if(result['item_name'][0] == "FMS006-2610-0(06)"){
				$('#txt_pn_code').val("QS0CZA359WJQZY");
			}else if(result['item_name'][0] == "FMS006-5100-0(04)"){
				$('#txt_pn_code').val("QS0CZA424WJQZY");
				$('#txt_qty').val('1300');
			}else if(result['item_name'][0] == "IC328-149-0001-VE"){
				$('#txt_pn_code').val("0850072859RYM00");
			}else if(result['item_name'][0] == "FPS009-2967-0(01)"){
				$('#txt_pn_code').val("P950K084P58");
				$('#txt_qty').val('500');
			}else if(result['item_name'][0] == "PJS008-2130-0(04)"){
				$('#txt_pn_code').val("P950K087P99");
				$('#txt_qty').val('500');
			}
