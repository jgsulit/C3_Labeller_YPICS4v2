<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 29
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		...
		<div id="container_label">
			<table id="tbl_format29" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_pn" id="txt_pn" tabindex="1" value="" readonly required>
					
					<label class="control-label">Manufacture P/N:</label>
					<input type="text" class="form-control" name="txt_mpn" id="txt_mpn" tabindex="3" value="" readonly required>
					...
					<div class="row">
						<div class="col-sm-6">
					        <?php								
								...
								// }
							?>    
<!--                            <input type="number" class="form-control" name="txt_qty" id="txt_qty" tabindex="2" value="400" readonly required>-->
						</div>
					</div>					
...
var handler = '../handler/handler_F29.php';
...
var oTable = $('#tbl_format29').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource"		: "../server_side/format29.php"
} );
...
function check_existing_item_name_and_lotno(pn,lotno){
	var data = {
		"action" 	: "check_existing_item_name_and_lotno_format29",
		"pn"		: pn,
		"lotno"		: lotno
	}
	data = $.param(data);
...
function frm_new_label(){
	var data = {
		"action"	: "save_new_c3_label_format29",
		"format_no"	: "29",
		"username"	: username,
	...
	$.ajax({
		...
		success	: function(result){
			...
			console.log("print",result['label']);
			...
			if ( ( formid ).indexOf('#') > -1){
				...
				switch( formid ){
					case 'frm_new_label':
						...
						console.log("frm_new_label", arr);
						validate_user(['4','1','2','5','7'],arr);
						break;
						case 'frm_new_label_partial':
							var arr = [];
							console.log("frm_new_label_partial", arr);
						arr['action'] = 'frm_new_label';
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format29",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format29 tbody').on('click','#chk_select', function(){
...
function default_values(device_name){
	$.ajax({
		...
		success	: function(result){
			...
			//-11242020
			if (result['item_name'] == 'CN176S-060-0002(01)'){
				$('#txt_pn').val('108205');
				$('#txt_mpn').val('CN176S-060-0002-01');
				$('#txt_qty').val('290');
			}else if (result['item_name'] == 'CA009-S101-001(08)'){
				$('#txt_pn').val('108205');
				$('#txt_mpn').val('CA009-S101-001-08');
				$('#txt_qty').val('240');
			}else if (result['item_name'] == 'FPS009-2903-0(10)'){
				$('#txt_pn').val('108205');
				$('#txt_mpn').val('FPS009-2903-0-10');
				$('#txt_qty').val('500');
			}else{
