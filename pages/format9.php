<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 9
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		...
		<div id="container_label">
			<table id="tbl_format9" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					<label class="control-label">IAC Part No. :</label>
					<input type="text" class="form-control" name="txt_iac_part_no" id="txt_iac_part_no" tabindex="1" required>
					<label class="control-label">Part No. :</label>
					<input type="text" class="form-control" name="txt_part_no" id="txt_part_no" tabindex="3">
				</div>
				<div class="col-sm-6">
...
var oTable = $('#tbl_format9').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource"		: "../server_side/format9.php"
} );
...
function create_datalist_item_name(){
	$.ajax({
		...
		success	: function(result){
			...
			$('#txt_yec_pn').attr('list','list_item_name');
		},error : function(){
			alert('error handler');
		}
...
$('#txt_lot_no').change(function(){
	var iac_part_no = $('#txt_iac_part_no').val();
	var lotno 		= $(this).val();
	check_existing_item_name_and_lotno(iac_part_no,lotno);
});

/* function for checking existing item name and lotno */
function check_existing_item_name_and_lotno(iac_part_no,lotno){
	var data = {
		"action" 		: "check_existing_item_name_and_lotno_format9",
		"iac_part_no"	: iac_part_no,
		"lotno"			: lotno
	}
	data = $.param(data);
	$.ajax({
...
$('#frm_new_label').submit(function(e){
	e.preventDefault();
	var data = {
		"action"	: "save_new_c3_label_format9",
		"username"	: username
	}
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format9",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format9 tbody').on('click','#chk_select', function(){
