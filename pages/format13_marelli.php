<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 13
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		<div class="btn-container">
			<p class="align-right">
				...
				<?php session_start(); ?>
				<?php								
					if($_SESSION['username'] == 'jojpato' || $_SESSION['username'] == 'ronfern'){
		...
		<div id="container_label">
			<table id="tbl_format13" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_pn" id="txt_pn" tabindex="1" readonly required>
					<label class="control-label">MM's No. :</label>
					<input type="text" class="form-control" name="txt_mmno" id="txt_mmno" tabindex="3" readonly required>
					<label class="control-label">DATE CODE :</label>
					<input type="date" class="form-control" name="txt_date_code" id="txt_date_code" tabindex="5">
				</div>
				<div class="col-sm-6">
...
var oTable = $('#tbl_format13').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource"		: "../server_side/format13.php"
} );
...
$('#frm_search').submit(function(e){
	...
	var search_pattern 		= $('#txt_search_pattern').val().trim();
...
function search_item(search_category,search_pattern){
	$.ajax({
		...
		success	: function(result){
			$('#modal_new_sticker #txt_pn').val('');
			if(result['exist'] == 0){
				alert('Record does not exist!');
				return false;
			}
			// $('#modal_new_sticker #txt_pn').val(result['item_name'][0].trim());
			$('#modal_new_sticker #txt_pn').val(result['item_name'][0].trim());
			$('#modal_search input').val('');
			$('#modal_search').modal('hide');
			default_values( result['item_name'][0] );

			// $('#modal_new_sticker #txt_pn').val('');//reset
			$('#modal_new_sticker #txt_mmno').val('');//reset
			$.ajax({
				type	: "post",
				dataType: "json",
				data	: 'action=search_mmno&pn2='+result['item_name'][0].trim(),
				url		: '../format13/handler/handler.php',
				success	: function(result){
					if(result['ctr']>0){
						$('#modal_new_sticker #txt_pn').val(result['pn'][0]);
						$('#modal_new_sticker #txt_mmno').val(result['mm_no'][0]);						
					}
				}, error : function(){
					alert('error handler');
				}
...
function default_values(item_name){
	// var isContains = item_name.indexOf('HF107P-0001-0') > -1;
	var isContains = item_name.indexOf('HF107P-1001-0') > -1;
	if(isContains){
		$('#modal_new_sticker #txt_qty').val('200');
	}
	else{
		isContains = item_name.indexOf('HF108P-1001-0') > -1;
		if(isContains){
			$('#modal_new_sticker #txt_qty').val('200');
		}
		else{
			isContains = item_name.indexOf('HF112P-0001-0') > -1;
			if(isContains){
				$('#modal_new_sticker #txt_qty').val('180');
			}
		}
	}
...
function check_existing_item_name_and_lotno(pn,lotno){
	var data = {
		"action" 		: "check_existing_item_name_and_lotno_format13",
		"pn"			: pn,
		"lotno"			: lotno
	}
	data = $.param(data);
...
$('#frm_new_label').submit(function(e){
	e.preventDefault();
	var data = {
		"action"	: "save_new_c3_label_format13",
		"username"	: username,
		"marelli"	: 1
	}
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format13",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format13 tbody').on('click','#chk_select', function(){
