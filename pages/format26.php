<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 26
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		...
		<div id="container_label">
			<table id="tbl_format26" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_pn" id="txt_pn" tabindex="1" value="9905016233" readonly required>
					...
					<input type="text" class="form-control" name="txt_mpn" id="txt_mpn" tabindex="3" value="CN121S-104-3002(H1)" readonly required>
...
var oTable = $('#tbl_format26').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource"		: "../server_side/format26.php"
} );
...
function check_existing_item_name_and_lotno(pn,lotno){
	var data = {
		"action" 	: "check_existing_item_name_and_lotno_format26",
		"pn"		: pn,
		"lotno"		: lotno
	}
	data = $.param(data);
...
function frm_new_label(){
	var data = {
		"action"	: "save_new_c3_label_format26",
		"format_no"	: "26",
		"username"	: username,
	...
	$.ajax({
		...
		success	: function(result){
			...
			$('#txt_pn').val('9905016233');	
			$('#txt_mpn').val('CN121S-104-3002(H1)');
			$('#txt_qty').val('240');
			get_work_week();
		}
	});
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format26",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format26 tbody').on('click','#chk_select', function(){
