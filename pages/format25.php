<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 25 (YFB-A121854, YFB-A121855, YFB-A121856)
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		...
		<div id="container_label">
			<table id="tbl_format25" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_pn_code" id="txt_pn_code" tabindex="1" required>
					<label class="control-label">Contractor Code:</label>
					<input type="text" class="form-control" name="txt_contractor_code" id="txt_contractor_code" tabindex="3">
					<label class="control-label">Spec:</label>
					<input type="text" class="form-control" name="txt_spec" id="txt_spec" tabindex="6">
					<label class="control-label">With P:</label>
					<input type="checkbox" class="" name="chk_p" id="chk_p" value="1" tabindex="7">
				</div>
				<div class="col-sm-6">
					...
					<div class="row">
						<div class="col-sm-4">
							<input type="number" class="form-control" name="txt_print_qty" id="txt_print_qty" tabindex="7" value="1" required>
						</div>
					</div>
				</div>
			</div>
      ...
      <div class="modal-body">
        <form id="frm_search">
			...
			<div class="row">
				...
				<div class="col-sm-7">
					<input type="text" class="form-control" id="txt_search_pattern" autocomplete="off">
				</div>						
			</div>
      </div>
      <div class="modal-footer">
...
var oTable = $('#tbl_format25').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource": "../server_side/format25.php"
} );
...
function create_datalist_item_name(){
	$.ajax({
		...
		success	: function(result){
			...
			$('#txt_spec').attr('list','list_item_name');
		},error : function(){
			alert('error handler');
		}
...
$('#txt_lot_no,#txt_spec').keyup(function(){
	var spec 		= $('#txt_spec').val();
	var lotno 		= $('#txt_lot_no').val();
	check_existing_item_name_and_lotno(spec,lotno);
});

/* function for checking existing item name and lotno */
function check_existing_item_name_and_lotno(spec,lotno){
	var data = {
		"action" 		: "check_existing_item_name_and_lotno_format25",
		"spec"			: spec,
		"lotno"			: lotno
	}
	data = $.param(data);
...
$('#frm_new_label').submit(function(e){
	e.preventDefault();
	var data = {
		"action"	: "save_new_c3_label_format25",
		"username"	: username
	}
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format25",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format25 tbody').on('click','#chk_select', function(){
...
function search_item(search_category,search_pattern){
	$.ajax({
		type	: "post",
		dataType: "json",
		data	: 'action=search_item_yf&search_category='+search_category+'&search_pattern='+search_pattern,
		url		: handler,
		success	: function(result){
			$('#txt_spec').val('');
			if(result['exist'] == 0){
			...
			var item_name_new = result['item_name'][0];
			var CODE = result['CODE'][0];
			item_name_new = item_name_new.split('PART No.')[0];
			item_name_new = item_name_new.trim();
			$('#txt_spec').val(item_name_new);
			$('#txt_contractor_code').val('108205');

			$('#modal_search input').val('');
			$('#modal_search').modal('hide');
            default_values(result['item_name'][0]);
		}, error : function(){
			alert('error handler on search_item() function');
		}
	});
}
...
function default_values(device_name){
	$.ajax({
		...
		success	: function(result){
			console.log(result);
			$('#txt_qty').val("0");//novs 2020-10-24 removed since no use
			$('#txt_qty').val(result['data']['quantity']);
			$('#txt_pn_code').val(result['data']['code']);//added for part number auto fetch from cn default, removed since there is other texts needed at the top of the bar code (not included in format 10)
		}, error : function(){
			alert('error handler on default_values() function');
		}
	});
}
