<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 27 (Revised Ver.)
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		<div class="btn-container">
			<p class="align-right">
				...
				<?php session_start(); ?>
				<?php								
					// if($_SESSION['username'] == 'jojpato' || $_SESSION['username'] == 'marlope' || $_SESSION['username'] == 'ronfern'){
						// print_r( $_SESSION );
		...
		<div id="container_label">
			<table id="tbl_format27" class="table table-bordered">
				<thead>
					<tr>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					<input type="hidden" class="form-control" name="txt_po_number" id="txt_po_number" tabindex="1" readonly required>
					<input type="hidden" class="form-control" name="txt_item_code" id="txt_item_code" tabindex="1" readonly required>
					<label class="control-label">Item Name:</label>
					<input type="text" class="form-control" name="txt_itemname" id="txt_itemname" tabindex="1" readonly required>
					<label class="control-label">Order No:</label>
					<input type="text" class="form-control" name="txt_order_no" id="txt_order_no" tabindex="3" readonly>
				...
				<div class="col-sm-6">
					...
					<div class="row">
						<div class="col-sm-6">
							<?php
								echo '<input type="number" class="form-control" name="txt_qty" id="txt_qty" tabindex="2" data-qty="0" readonly required>';
					...
					<div class="row" id="container_lot_number">
						...
						<div class="col-sm-4 no-padding"> 
							<input type="text" class="form-control" name="txt_lot_no_count" id="txt_lot_no_count" maxlength="4" placeholder="" required>
						</div>
					</div>
			...
			<div id="sticker_preview">
				<img src="../img/C3_Label_Format/formatna.jpg" class="image">
...
var oTable = $('#tbl_format27').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource": "../server_side/format27.php"
} );
...
function check_existing_item_name_and_lotno(item_name,lotno){
	var data = {
		"action" 	: "check_existing_item_name_and_lotno_format27",
		"item_name"	: item_name,
		"lotno"		: lotno
	}
	data = $.param(data);
...
function frm_new_label(){
	var data = {
		"action"	: "save_new_c3_label_format27",
		"username"	: username,
		"txt_employee_number_scanner"	: $('#txt_employee_number_scanner').val(),
		"txt_item_code"	: $('#txt_item_code').val(),
		"txt_po_number"	: $('#txt_po_number').val(),
...
$('#btn_reprint_label').click(function(){
	...
	load_selected_label_info_format27('#tbl_selected_labels_re_print');
	/* display modal */
...
$('#btn_delete_label').click(function(){
	...
	load_selected_label_info_format27('#tbl_selected_labels');
...
function load_selected_label_info_format27(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info_format27",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format27 tbody').on('click','#chk_select', function(){
...
function search_item(search_category,search_pattern){
	$.ajax({
		...
		success	: function(result){
			...
			if(result['item_name'][0]=='HF301S-106-1800-4021-VE'){
				alert('Please use Format 22 for HF301S-106-1800-4021-VE');
				return false;
			}

			$('#txt_po_number').val(result['PONo'][0]);
			$('#txt_itemname').val(result['item_name'][0]);
			$('#txt_item_code').val(result['item_code'][0]);
			console.log((result['item_code'][0]));
			console.log((result['PONo'][0]));
