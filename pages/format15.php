<div class="main_container">
	<legend> 
		<h1>
			<span class="fa fa-book"> Format 15
			</span>
		</h1>
	</legend>
	...
	<div class="panel">
		...
		<div id="container_label">
			<table id="tbl_format15" class="table table-bordered" style="font-size:12px;">
				<thead>
					<tr>
        ...
        <form id="frm_new_label">
			<div class="row">
				<div class="col-sm-6">
					...
					<input type="text" class="form-control" name="txt_pono" id="txt_pono" tabindex="1" readonly required>
					<label class="control-label">Part Code:</label>
					<input type="text" class="form-control" name="txt_partcode" id="txt_partcode" tabindex="2" readonly required>
					
					<label class="control-label">Part Name:</label>
					<input type="text" class="form-control" name="txt_partname" id="txt_partname" tabindex="3" readonly required>
				</div>
				<div class="col-sm-6">
					...
					<div class="row">
						<div class="col-sm-6">
                            <?php								
								// if($_SESSION['username'] == 'jojpato' || $_SESSION['username'] == 'marlope'){
									echo '<input required type="number" class="form-control" name="txt_qty" id="txt_qty" tabindex="2" required>';
								// }else{
									// echo '<input type="number" class="form-control" name="txt_qty" id="txt_qty" tabindex="2" required readonly>';
								// }
							?>
<!--							<input type="number" class="form-control" name="txt_qty" id="txt_qty" value="" tabindex="2" required>-->
						</div>
					</div>					
					<label class="control-label">Lot Number:</label>
					<input type="text" class="form-control" name="txt_lot_no" id="txt_lot_no" tabindex="4" required>
			...
			<div id="sticker_preview">
				...
				<!--<p class="align-center"><img src="../img/C3_Label_Format/format15.jpg" class="image"></p>
				<div id="preview_item_name">CN048S-0002-0</div>
				<div id="preview_qty">2000pcs</div>
				<div id="preview_lot_no">5Z22-14A</div>
				<div id="preview_product_name">450123456700010</div>
      ...
      <div class="modal-body">
        <form id="frm_search">
			<div class="row">
				...
				<div class="col-sm-7">
					<select class="form-control" id="cmb_search_category" readonly>
...
var oTable = $('#tbl_format15').DataTable( {
	"bProcessing": true,
	"bServerSide": true,
	"aaSorting"	 : [],
	"sAjaxSource": "../server_side/format15.php"
} );
...
function check_existing_item_name_and_lotno(item_name,lotno){
	var data = {
		"action" 	: "check_existing_item_name_and_lotno15",
		"item_name"	: item_name,
		"lotno"		: lotno
	}
	data = $.param(data);
...
$('#frm_new_label').submit(function(e){
	...
	$.ajax({
		...
		success	: function(result){
			...
			$('#frm_new_label input[name=txt_lot_no]').val('');
			$('#frm_new_label input[name=txt_lot_no_count]').val('');
		}
	});
});
...
function load_selected_label_info(tbl_id){
	var data = {
		"action"	 : "load_selected_label_info15",
		"array_pkid" : array_selected
	}
	data = $.param(data);
	$.ajax({
...
$('#tbl_format15 tbody').on('click','#chk_select', function(){
...
function search_item(search_category,search_pattern){
	$.ajax({
		type	: "post",
		dataType: "json",
		data	: 'action=search_item_cn&search_category='+search_category+'&search_pattern='+search_pattern,
		url		: handler,
		success	: function(result){
			console.log(result);
			// $('#txt_po_no').val('');
			...
			// $('#txt_product_name').val(result['item_name'][0]);
			$('#txt_pono').val(result['PONumber']);
			$('#txt_partcode').val(result['DeviceCode']);
			$('#txt_partname').val(result['DeviceName']);
			/* MJ add mo as placevalue sa qty yung PO Qty */
			// $('#txt_partname').attr('').val(result['DeviceName']);
			$('#modal_search input').val('');
			$('#modal_search').modal('hide');
            default_values( result['DeviceName'] );
		}, error : function(){
			alert('error handler');
		}
	});
